...
import { useCompletedWithdrawHistoryQuery } from "../../../../services/AdminWithdraw";

const SuccessfulWithdraw = () => {
  const { data, isLoadingCompletedWithdrawHistory } =
  ...
  if (isLoadingCompletedWithdrawHistory) {
    return <Loading />;
  }
  return (
    <>
      <SectionCommonTable
        ...
        sectionTableTitle={`Success Withdraw History (${
          filterData?.length > 0
            ? filterData?.length
